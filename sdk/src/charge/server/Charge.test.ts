...
describe('charge request transform', () => {
  ...
  it('advertises credentialTypes excluding legacy hash by default (rejectUnsignedPush: true)', () => {
    const method = charge({
      recipient: RECIPIENT,
      currency: USDC_SAC_TESTNET,
      store: Store.memory(),
  ...
  it('advertises credentialTypes including legacy hash when rejectUnsignedPush is explicitly false', () => {
    const method = charge({
      recipient: RECIPIENT,
      currency: USDC_SAC_TESTNET,
      store: Store.memory(),
...
describe('charge legacy hash (unsigned push) handling', () => {
  ...
  it('accepts legacy hash when explicitly opted in with rejectUnsignedPush: false and logs deprecation warning', async () => {
    const client = PAYER
    const tx = buildTransferTx({
      source: client.publicKey(),
      from: client.publicKey(),
  ...
  it('still rejects legacy hash when rejectUnsignedPush is explicitly true (now the default)', async () => {
    const client = PAYER
    const tx = buildTransferTx({
      source: client.publicKey(),
      from: client.publicKey(),
