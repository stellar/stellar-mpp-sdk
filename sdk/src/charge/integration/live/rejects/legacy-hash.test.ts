...
describe('legacy hash credential rejection (rejectUnsignedPush: true)', () => {
  beforeAll(async () => {
    await Promise.all([
  ...
  it('strict server rejects legacy hash credential without consuming hash', async () => {
    // Create strict server method (rejectUnsignedPush: true)
