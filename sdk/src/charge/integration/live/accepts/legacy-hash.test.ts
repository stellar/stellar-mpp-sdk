...
describe('legacy hash credential acceptance (rejectUnsignedPush: false opt-in)', () => {
  beforeAll(async () => {
    await Promise.all([
  ...
  it('server with explicit rejectUnsignedPush: false accepts legacy hash credential and settles on-chain', async () => {
    // Create server method with explicit rejectUnsignedPush: false for legacy acceptance
