...
- Raise the `mppx` peer dependency to `^0.9.3` (from `^0.8.1`) — consumers must bump. `mppx@0.9.3` in turn raises its `viem` peer minimum to `>=2.54.0`; the development `viem` dependency here moves to `^2.54.0` to match. `mppx@0.10.1` is the latest release but hasn't yet cleared this repo's configured 7-day `minimumReleaseAge` soak (`pnpm-workspace.yaml`); a follow-up bump to 0.10.x can land once it does [#78](https://github.com/stellar/stellar-mpp-sdk/pull/78)
