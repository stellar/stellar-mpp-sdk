...
- Stamp every charge challenge with a server-generated `methodDetails.reference` nonce and an `issuedAt` timestamp. The challenge id is an HMAC over the challenge contents, and a charge request carried no per-issuance value of its own: two customers buying the same resource at the same price collided on the single-use replay slot whenever their challenges were issued in the same millisecond, and every challenge on a route with an operator-set fixed `expires` shared one id permanently. In push mode the second customer had already settled on-chain when the server rejected them as a replay. Channel mode already carried this nonce [#80](https://github.com/stellar/stellar-mpp-sdk/pull/80)
- Release the charge challenge claim in push mode (`signedHash` and legacy `hash`) when verification fails before the payment's tx-hash slot is consumed, matching the pull-mode behaviour. A transient RPC failure or a not-yet-confirmed lookup no longer locks a paid-up customer out of their own challenge, and a stranger can no longer burn a challenge slot with a credential for a non-existent transaction [#80](https://github.com/stellar/stellar-mpp-sdk/pull/80)
- Anchor push-mode payment freshness to the challenge's stamped `issuedAt` instead of inferring issuance from `expires` minus the configured lifetime. A route with an operator-set expiry longer than the default lifetime previously rejected every push payment as predating issuance after the client had already paid. Challenges without the stamp fall back to the previous derivation [#80](https://github.com/stellar/stellar-mpp-sdk/pull/80)
