...
- Verify channel vouchers against the `CommitmentKey` stored in the channel contract's instance storage rather than only against the configured `commitmentKey`. The on-chain key is read once per server and cached; a configured `commitmentKey` that differs from it rejects every voucher. Previously the server trusted the operator-supplied key alone, so a funder could deploy a channel with one key, hand the recipient another, and obtain service with vouchers the contract would never settle, then reclaim the full deposit after the refund waiting period [#76](https://github.com/stellar/stellar-mpp-sdk/pull/76)
